...
public class GetMonthlyBillServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private BillingServices services;
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			Bill bill=services.getMobileBillDetails(Integer.parseInt(request.getParameter("customerId")), Long.parseLong(request.getParameter("mobileNumber")), request.getParameter("month"));
			request.setAttribute("bill", bill);
			request.getRequestDispatcher("monthlyBillPage.jsp").forward(request, response);
